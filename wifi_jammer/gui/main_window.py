...
from wifi_jammer.core.interfaces import NetworkInfo, AttackConfig, AttackType
...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self) -> None:
        ...
        self.attack_config_widget = AttackConfigWidget(
            self.attack_factory, self.logger
        )
    ...
    def _check_permissions(self) -> None:
        ...
        if os.geteuid() != 0 and sys.platform != 'win32':
            self.status_bar.showMessage(
                "Warning: May need root privileges for full functionality",
                5000
            )
    ...
    def _on_network_selected(self, network: NetworkInfo) -> None:
        ...
        self.status_bar.showMessage(
            f"Network selected: {network.ssid} ({network.bssid})",
            3000
        )
    ...
    def _on_attack_started(self, config: AttackConfig) -> None:
        ...
        self.status_bar.showMessage(
            f"Attack started: {config.attack_type.value}",
            3000
        )
