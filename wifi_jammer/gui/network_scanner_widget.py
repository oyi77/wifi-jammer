    ...
    def _update_network_table(self, networks: List[NetworkInfo]) -> None:
        ...
        for row, network in enumerate(networks):
            ...
            self.network_table.setItem(
                row, 2, QTableWidgetItem(str(network.channel))
            )
            self.network_table.setItem(
                row, 3, QTableWidgetItem(f"{network.rssi} dBm")
            )
            ...
            self.network_table.setItem(
                row, 5, QTableWidgetItem(str(clients_count))
            )
    
    def _on_network_double_clicked(self, index) -> None:
