...
Encapsulates every macOS-only code path of the scanner: CoreWLAN
framework calls, ``wdutil``/``airport``/``system_profiler`` output
parsing, and Location Services permission handling.

This class never talks to the network itself via scapy — the standard
scapy scanning lives in ``ScapyNetworkScanner`` and is injected here as
the ``scapy_fallback`` callable for the root-only scan path.
"""
...
class MacOSScanner:
    """All macOS-specific network discovery, parsing, and permission logic."""
    ...
    def get_current_network(self, interface: str) -> Optional[NetworkInfo]:
        ...
        try:
            ...
            corewlan_network = self.get_current_network_via_corewlan(interface)
            if corewlan_network:
                self.logger.info(
            ...
            if not self.state.networks:
                try:
                    ...
                    if result.returncode == 0:
                        self.parse_macos_networks(result.stdout)
