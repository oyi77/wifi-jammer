...
from wifi_jammer.core.interfaces import INetworkScanner, NetworkInfo
...
def _install_corewlan_if_needed():
    ...
    try:
        ...
        result = sp.run([python_exe, "-m", "pip", "install", "pyobjc-framework-CoreWLAN"], 
        ...
        if result.returncode == 0:
            ...
            print(f"⚠️  Failed to install CoreWLAN:")
            if result.stdout:
                print(f"   stdout: {result.stdout}")
            if result.stderr:
                print(f"   stderr: {result.stderr}")
            print(f"   Try installing manually: {python_exe} -m pip install pyobjc-framework-CoreWLAN")
    except Exception as e:
        print(f"⚠️  Could not auto-install CoreWLAN: {e}")
        print("   You can install it manually with: pip install pyobjc-framework-CoreWLAN")
...
class ScapyNetworkScanner(INetworkScanner):
    ...
    def _check_python_permission(self):
        ...
        if self._python_with_permission and self._python_with_permission != sys.executable:
            self.logger.debug(f"Found Python with permission: {self._python_with_permission}")
        elif not self._python_with_permission:
            # Try to request permission
            self._request_permission_if_needed()
    
    def _request_permission_if_needed(self):
        ...
        try:
            ...
            self.logger.info("   Or run: bash install.sh (which runs setup automatically)")
    ...
    def get_interface_list(self) -> List[str]:
        ...
        try:
            ...
            if not wireless_interfaces:
                ...
                self.logger.warning(f"No wireless interfaces detected on {platform_type}")
                
                # Provide helpful diagnostics for macOS
                if platform_type == "macos":
                    self.logger.info("💡 macOS Tips:")
                    self.logger.info("   - Make sure Wi-Fi is enabled in System Settings")
                    self.logger.info("   - Try running: networksetup -listallhardwareports")
            ...
            if not interface_names:
                ...
                unavailable = [iface.name for iface in wireless_interfaces if iface.status != "Available"]
                if unavailable:
                    self.logger.info(f"Found interfaces but they are not available: {', '.join(unavailable)}")
                ...
                for iface in wireless_interfaces:
                    if iface.name in interface_names:
                        if not iface.is_monitor_capable:
                            self.logger.debug(f"Interface {iface.name}: Monitor mode not supported (this is normal on macOS)")
        ...
        try:
            ...
            if is_macos():
                ...
                if _COREWLAN_AVAILABLE:
                    try:
                        self.logger.info("🔍 Scanning networks using CoreWLAN framework...")
                        corewlan_count = self._scan_macos_networks_via_corewlan(interface, channel)
                        if corewlan_count > 0:
                            self.logger.info(f"✅ CoreWLAN scan successful, found {corewlan_count} network(s)")
                    except (ImportError, AttributeError, OSError, RuntimeError) as e:
                        self.logger.debug(f"CoreWLAN scan failed: {e}, trying fallback methods...")
                ...
                try:
                    ...
                    if current_network:
                        self.logger.info(f"Found current network: {current_network.ssid} ({current_network.bssid})")
                        with self._lock:
                            # Only add if not already in list
                            if not any(n.bssid == current_network.bssid or n.ssid == current_network.ssid for n in self._networks):
                ...
                try:
                    ...
                    if is_root:
                        try:
                            self.logger.info("Attempting basic scapy-based scan (requires root)...")
                        ...
                        self.logger.info("💡 Tip: Run with sudo for scapy-based scanning (more accurate results)")
            else:
    ...
    def _get_current_macos_network(self, interface: str) -> Optional[NetworkInfo]:
        ...
        try:
            ...
            if corewlan_network:
                self.logger.info(f"✅ CoreWLAN successfully retrieved network: {corewlan_network.ssid}")
                return corewlan_network
            else:
                self.logger.debug("CoreWLAN did not return network info, trying fallback methods...")
            
            # Step 1: Try airport -I first (gives current network info, may require sudo)
            airport_path = "/System/Library/PrivateFrameworks/Apple80211.framework/Versions/Current/Resources/airport"
            if os.path.exists(airport_path):
                ap_result = subprocess.run([airport_path, interface, '-I'], capture_output=True, text=True, timeout=5)
                if ap_result.returncode == 0 and ap_result.stdout.strip():
                    ...
                    bssid_match = re.search(r'^\s*BSSID:\s*([0-9a-fA-F:]+)$', output, re.MULTILINE)
                    channel_match = re.search(r'^\s*channel:\s*(\d+)', output, re.MULTILINE | re.IGNORECASE)
                    ...
                    if ssid_match and bssid_match:
                        ...
                        sp_result = subprocess.run(['system_profiler', 'SPAirPortDataType'], 
            ...
            if sp_result.returncode == 0:
                ...
                ifconfig_result = subprocess.run(['ifconfig', interface], capture_output=True, text=True, timeout=5)
                has_ip = 'inet ' in ifconfig_result.stdout if ifconfig_result.returncode == 0 else False
                ...
                if has_ip:
                    ...
                    ns_result = subprocess.run(['networksetup', '-getairportnetwork', interface], 
                                              capture_output=True, text=True, timeout=5)
                    if ns_result.returncode == 0:
                        ns_output = ns_result.stdout.strip()
                        if "not associated" not in ns_output.lower() and ":" in ns_output:
                        ...
                        for i, line in enumerate(lines):
                            if "Current Network Information:" in line:
                                ...
                                for j in range(i, min(i+20, len(lines))):
                                    if lines[j].strip().endswith(':') and '<redacted>' not in lines[j]:
                                        ...
                                        if potential_ssid and len(potential_ssid) > 0:
                                            ...
                                            if not any(x in potential_ssid for x in ['PHY Mode', 'Channel', 'Country', 'Network Type', 'Security', 'Signal', 'Transmit', 'MCS']):
                                                ssid = potential_ssid
                                                break
                                break
                ...
                for i, line in enumerate(lines):
                    ...
                    if in_current_section:
                        ...
                        # Extract RSSI/Signal (format: "Signal / Noise: -74 dBm / -94 dBm")
                            ...
                            rssi_match = re.search(r'Signal[^:]*:\s*(-?\d+)\s*dBm', line_stripped)
                            if rssi_match:
                        ...
                        elif line_stripped and not line_stripped.startswith(' ') and ':' in line_stripped and "Other" in line_stripped:
                            break
            ...
            for cmd in [['wdutil', 'info'], ['sudo', 'wdutil', 'info']]:
                try:
                    wd_result = subprocess.run(cmd, capture_output=True, text=True, timeout=5)
                    if wd_result.returncode == 0:
                        bssid_match = re.search(r'BSSID\s+:\s+([0-9a-fA-F:]+)', wd_result.stdout)
                        if bssid_match:
                            potential_bssid = bssid_match.group(1)
                            # Only use if it's not redacted and looks like a real MAC
                            if potential_bssid != "<redacted>" and len(potential_bssid) == 17:
                                bssid = potential_bssid
                                # Also get channel and RSSI from wdutil if available
                                if not channel or channel == 0:
                                    ch_match = re.search(r'Channel\s+:\s+([0-9a-z/]+)', wd_result.stdout)
                                    if ch_match:
                                        ch_str = ch_match.group(1)
                                        if 'g' in ch_str:
                                            channel = int(ch_str.split('g')[1].split('/')[0])
                                        else:
                                            channel = int(ch_str.split('/')[0])
                                if not rssi or rssi == -50:
                                    rssi_match = re.search(r'RSSI\s+:\s+(-?\d+)\s*dBm', wd_result.stdout)
                                    if rssi_match:
                                        rssi = int(rssi_match.group(1))
                                break
            ...
            airport_path = "/System/Library/PrivateFrameworks/Apple80211.framework/Versions/Current/Resources/airport"
            if os.path.exists(airport_path):
                ap_result = subprocess.run([airport_path, '-s'], capture_output=True, text=True, timeout=10)
                if ap_result.returncode == 0 and ap_result.stdout.strip() and "WARNING" not in ap_result.stdout:
                    ...
                    for line in ap_result.stdout.split('\n'):
                        ...
                        if bssid_match:
                            ...
                            ssid_match = re.match(r'^(.+?)\s+' + re.escape(line_bssid), line)
                            line_ssid = ssid_match.group(1).strip() if ssid_match else None
                            
                            # Extract RSSI and channel
                            ch_rssi_match = re.search(r'\s+(-?\d+)\s+(\d+)', line)
                            line_rssi = int(ch_rssi_match.group(1)) if ch_rssi_match else None
                            line_channel = int(ch_rssi_match.group(2)) if ch_rssi_match else None
                            
                            # If we have a BSSID from wdutil, match by BSSID
                            if bssid and bssid != "<redacted>" and line_bssid.lower() == bssid.lower():
                                if line_ssid:
                                    ssid = line_ssid
                                if line_rssi:
                                    rssi = line_rssi
            ...
            if (bssid and bssid != "<redacted>" and bssid != "00:00:00:00:00:00") or channel > 0:
                if not bssid or bssid == "<redacted>":
                    # Can't proceed without BSSID
                    self.logger.warning("Could not get BSSID for current network (privacy settings may be blocking it)")
                    return None
    ...
    def _get_current_macos_network_via_corewlan(self, interface: str) -> Optional[NetworkInfo]:
        ...
        try:
            ...
            from CoreWLAN import CWInterface, CWWiFiClient  # type: ignore
            ...
            if interface_obj:
                ...
                self.logger.debug(f"CoreWLAN interface power: {power_on}, associated: {associated}")
                ...
                if not associated:
                    self.logger.debug("CoreWLAN: Interface is not associated with a network")
                    return None
                ...
                self.logger.debug(f"CoreWLAN raw values - SSID: {ssid}, BSSID: {bssid}, RSSI: {rssi}")
                ...
                if ssid and bssid:
                    self.logger.info(f"✅ CoreWLAN found network: {ssid} ({bssid}) on channel {channel_number}")
                    return NetworkInfo(
                        ssid=ssid,
                        bssid=bssid,
                    ...
                    self.logger.warning("⚠️  CoreWLAN: SSID/BSSID are None - privacy settings are blocking access")
                    self.logger.info("")
                    self.logger.info("🔧 QUICK FIX:")
                    self.logger.info("   Run: python3 fix_location_permission.py")
                    self.logger.info("")
                    self.logger.info("📋 Or manually:")
                    self.logger.info("   1. System Settings → Privacy & Security → Location Services")
                    self.logger.info("   2. Enable Location Services (toggle ON)")
                    self.logger.info("   3. Find 'Terminal' or 'Python' and check the box ✅")
    ...
    def _scan_macos_networks_via_corewlan(self, interface: str, channel: Optional[int] = None) -> int:
        ...
        try:
            from CoreWLAN import CWInterface, CWWiFiClient, CWNetwork  # type: ignore
            ...
            with self._lock:
                ...
                for network in networks_set:
                    try:
                        ...
                        ssid = network.ssid()
                        bssid = network.bssid()
                        rssi = network.rssiValue() if hasattr(network, 'rssiValue') else -50
                        
                        # Get channel
                        wlan_channel = network.wlanChannel() if hasattr(network, 'wlanChannel') else None
                        channel_number = wlan_channel.channelNumber() if wlan_channel else 0
                        ...
                        if ssid and bssid:
                            ...
                            if not any(n.bssid == bssid for n in self._networks):
                                ...
                                networks_found += 1
                                self.logger.debug(f"CoreWLAN found: {ssid} ({bssid}) on channel {channel_number}")
                        else:
                            # Network found but SSID/BSSID are None (privacy restrictions)
                            self.logger.debug(f"CoreWLAN: Network found on channel {channel_number} but SSID/BSSID are blocked by privacy settings")
                    except Exception as e:
            ...
            if networks_found > 0:
                ...
                self.logger.warning(f"⚠️  CoreWLAN found {network_count} network(s) but SSID/BSSID are blocked by privacy settings")
                self.logger.info("")
                self.logger.info("🔧 QUICK FIX:")
                ...
                self.logger.info("   1. System Settings → Privacy & Security → Location Services")
                self.logger.info("   2. Enable Location Services (toggle ON)")
                self.logger.info(f"   3. Find '{os.path.basename(sys.executable)}' or 'Terminal' and check the box ✅")
    ...
    def _scan_macos_networks(self, interface: str, channel: Optional[int] = None):
        ...
        try:
            ...
            try:
                ...
                wd_cmd = ['wdutil', 'scan']
                result = subprocess.run(wd_cmd, capture_output=True, text=True, timeout=15)
                ...
                try:
                    ...
                    if result.returncode == 0 and result.stdout.strip():
                        ...
                        if has_networks:
                            ...
                            if networks_found > 0:
                                self.logger.info(f"✅ Airport utility found {networks_found} network(s)")
                                return
                        else:
                            self.logger.debug("Airport utility returned no network data (may be deprecated)")
            ...
            if not self._networks:
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        if len(self._networks) > 0:
                            self.logger.info(f"✅ system_profiler found {len(self._networks)} network(s)")
                            return
                except Exception as e:
            ...
            if is_root:
                try:
                    self.logger.info("Using scapy-based network scanning (running as root)...")
                    self._scan_standard_networks(interface, channel)
                    if len(self._networks) > 0:
                        return
                except Exception as e:
                ...
                if not self._networks:
                    ...
                    self.logger.info("   This enables scapy-based scanning (more accurate)")
    ...
    def _parse_wdutil_scan(self, output: str) -> int:
        ...
        try:
            ...
            for line in lines:
                ...
                # Look for BSSID
                bssid_match = re.search(r'BSSID\s*:\s*([0-9a-fA-F:]+)', line, re.IGNORECASE)
                if bssid_match:
                    bssid = bssid_match.group(1).strip()
                    if re.match(r'^[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}$', bssid):
                ...
                if 'ssid' in current_network and 'bssid' in current_network:
                    ...
                    with self._lock:
                        if not any(n.bssid == network_info.bssid for n in self._networks):
                            self._networks.append(network_info)
                            networks_found += 1
                            self.logger.debug(f"wdutil found: {network_info.ssid} ({network_info.bssid})")
    ...
    def _parse_airport_scan(self, output: str) -> int:
        ...
        try:
            ...
            for line in lines[1:]:
                ...
                match = re.search(r'^(.+?)\s+([0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2})\s+(-?\d+)\s+(\d+)', line)
                if match:
                    ...
                    if not re.match(r'^[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}:[0-9a-fA-F]{2}$', bssid):
                        continue
                    ...
                    with self._lock:
                        if not any(n.bssid == bssid for n in self._networks):
                            self._networks.append(network_info)
                            networks_found += 1
                            self.logger.debug(f"Airport found: {ssid} ({bssid}) on channel {channel}")
        except Exception as e:
    ...
    def _parse_macos_networks(self, output: str):
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                if "Current Network Information:" in line_stripped or "Current Network:" in line_stripped:
                ...
                if in_current_network:
                    ...
                    if "Network Name:" in line_stripped or "SSID:" in line_stripped:
                        current_network_data['ssid'] = line_stripped.split(':')[-1].strip()
                        ...
                        # End of current network section
                        if current_network_data.get('ssid') and current_network_data.get('bssid'):
                            network_info = NetworkInfo(
                                ssid=current_network_data.get('ssid', 'Unknown'),
                                bssid=current_network_data.get('bssid'),
                                channel=current_network_data.get('channel', 0),
                                rssi=current_network_data.get('rssi', -50),
                                encryption=current_network_data.get('encryption', 'Unknown'),
                                clients=[]
                            )
                            with self._lock:
                                if not any(n.bssid == network_info.bssid for n in self._networks):
                ...
                if in_networks_section:
                    ...
                    if line.endswith(':') and not any(keyword in line for keyword in ['Card Type', 'Firmware', 'MAC Address', 'Supported', 'Wake On', 'AirDrop', 'Auto Unlock', 'Status', 'Current Network', 'Other Local', 'Software Versions']):
                        ...
                        if network_name:
                            ...
                            if network_info:
                                with self._lock:
                                    if not any(n.ssid == network_name for n in self._networks):
                                        self._networks.append(network_info)
    ...
    def _scan_standard_networks(self, interface: str, channel: Optional[int] = None):
        ...
        try:
            ...
            is_root = os.geteuid() == 0 if hasattr(os, 'geteuid') else False
            if not is_root:
                self.logger.warning("⚠️  Scapy scanning requires root privileges on macOS")
                ...
                if interface_info.is_monitor_capable:
                    try:
                        if not self._platform_interface.set_monitor_mode(interface):
                            self.logger.debug(f"Could not set {interface} to monitor mode, scanning in managed mode...")
                    except Exception as e:
                        self.logger.debug(f"Monitor mode setup failed: {e}, continuing in managed mode...")
                else:
                    self.logger.debug(f"Interface {interface} does not support monitor mode, scanning in managed mode...")
            else:
            ...
            channels_to_scan = [channel] if channel else [1, 6, 11, 36, 40, 44, 48, 149, 153, 157, 161]
            ...
            self.logger.info("   Scapy requires root privileges on macOS. Run with sudo or use airport utility.")
        except Exception as e:
    ...
    def _scan_thread(self, interface: str, channel: Optional[int] = None):
        ...
        try:
            ...
            if is_macos_platform and interface_info and not interface_info.is_monitor_capable:
                self.logger.debug("macOS: Scanning without monitor mode (limited functionality)")
            ...
            for scan_channel in channels_to_scan:
                try:
                    ...
                    if interface_info and interface_info.is_monitor_capable and scan_channel:
                        try:
                            self._platform_interface.set_channel(interface, scan_channel)
                            time.sleep(0.2)  # Give interface time to switch channels
                        except Exception as e:
                            self.logger.debug(f"Could not set channel {scan_channel}: {e}")
                ...
                if channel or (ssid and ssid.strip()):
                    ...
                    with self._lock:
                        ...
                        if existing is not None:
                            ...
                            if network_info.rssi is not None and existing.rssi is not None:
                                if network_info.rssi > existing.rssi:  # Update with better signal
    ...
    def _process_data_packet(self, pkt):
        ...
        try:
            ...
            for addr in [pkt.addr1, pkt.addr2]:
                if addr and addr != "ff:ff:ff:ff:ff:ff" and addr.lower() != bssid.lower():
            ...
            if client_macs:
                with self._lock:
                    network = next((n for n in self._networks if n.bssid == bssid), None)
    ...
    def scan_clients(self, interface: str, ap_bssid: str, channel: Optional[int] = None, duration: int = 30) -> Dict[str, float]:
        ...
        def packet_handler(pkt):
            ...
            try:
                ...
                for addr in [pkt.addr1, pkt.addr2]:
                    if addr and addr != "ff:ff:ff:ff:ff:ff" and addr.lower() != ap_bssid.lower():
                ...
                if pkt.haslayer(Dot11) and pkt.type == 2:
                    ...
                    if hasattr(pkt, 'addr2') and pkt.addr2 and pkt.addr2.lower() != ap_bssid.lower():
        ...
        try:
            ...
            # Start sniffing
            self.logger.info(f"Scanning for clients on {ap_bssid} for {duration} seconds...")
            self.logger.info("Tip: Generate traffic on other devices to discover them faster")
