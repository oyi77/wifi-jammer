...
class ModernCrypto:
    ...
    def encrypt_aes(data: bytes, key: bytes, mode: str = "GCM") -> Dict[str, bytes]:
        ...
        if mode.upper() == "GCM":
            ...
            ciphertext = cbc_encryptor.update(padded_data) + cbc_encryptor.finalize()
    ...
    @staticmethod
    def decrypt_aes(
        encrypted_data: Dict[str, bytes], key: bytes, mode: str = "GCM"
    ) -> bytes:
        ...
        if mode.upper() == "GCM":
            ...
            cbc_cipher = Cipher(algorithms.AES(key), modes.CBC(encrypted_data["iv"]))
            cbc_decryptor = cbc_cipher.decryptor()

            padded_plaintext = (
                cbc_decryptor.update(encrypted_data["ciphertext"])
                + cbc_decryptor.finalize()
            )
    ...
    def verify_rsa_signature(
        ...
        try:
            ...
            return True
        except (ValueError, TypeError, AttributeError, OSError, Exception):
            return False
