...
class RichLogger(ILogger):
    ...
    def packet_sent(self, packet_num: int, target: Optional[str] = None) -> None:
        """Log packet sent with target information."""
        if self.quiet:
            return
        target_info = f" to {target}" if target else ""
        packet_text = Text(
            f"📦 Packet #{packet_num:,} sent{target_info}", style="green"
        )
    ...
    def attack_stopped(self, stats: Optional[Dict[str, Any]] = None) -> None:
