...
class TestCommandAvailability(unittest.TestCase):
    ...
    def test_check_commands_available(self, mock_which):
        ...
        available, missing = check_commands_available(
            ["python3", "pip3", "nonexistent"]
        )
...
class TestRootStatus(unittest.TestCase):
    ...
    def test_get_root_status_unix_root(
        self, mock_getuid, mock_geteuid, mock_is_windows
    ):
    ...
    @patch("os.geteuid", create=True)
    @patch("os.getuid", create=True)
    def test_get_root_status_unix_not_root(
        self, mock_getuid, mock_geteuid, mock_is_windows
    ):
