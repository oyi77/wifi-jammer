...
def install_dependencies(python_exe):
    ...
    run_command(f"{python_exe} -m pip install -q -r requirements.txt", check=False)

    # Install macOS-specific dependencies
    if platform.system() == "Darwin":
        print("   Installing macOS-specific dependencies...")
        run_command(
            f"{python_exe} -m pip install -q pyobjc-framework-CoreWLAN pyobjc-framework-CoreLocation",
            check=False,
        )
...
def setup_macos():
    ...
    print(
        f'  - Run: {current_python} -c "from CoreWLAN import CWWiFiClient; CWWiFiClient.sharedWiFiClient().interface().ssid()"'
    )
